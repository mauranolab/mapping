...
if [[ "${processingCommand}" =~ ^map ]]; then
    ...
    qsub -S /bin/bash -cwd -V ${qsubargs} -pe threads ${mapThreads} --time 5-0:00:00 --mem-per-cpu 15G -terse -j y -b y -t 1-${n} -o ${sampleOutdir} -N map.${mapname} "${src}/map.sh ${genomesToMap} ${analysisType} ${sampleOutdir} '${sampleIDs}' ${src}" | perl -pe 's/[^\d].+$//g;' > ${sampleOutdir}/sgeid.map.${mapname}
fi
