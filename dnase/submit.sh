...
echo "$name hotspots2"
qsub -p -450 -S /bin/bash -cwd -V --qos=normal -pe threads 2 -terse -j y -b y -hold_jid `cat $sampleOutdir/sgeid.makeTracks.$name` -o $sampleOutdir/ -N hotspot2.$name "$src/hotspot2.sh $name" |perl -pe 's/[^\d].+$//g;'  > $sampleOutdir/sgeid.hot2.$name
