...
#zcat /gpfs/data/isg/annotation/fasta/mm10/mm10.fa.gz | grep -v "^>" | perl -pe 's/[ACGTN\n]+//ig;' | wc -c
#
...
chromsizes="/gpfs/data/isg/annotation/fasta/${mappedgenome}/${mappedgenome}.chrom.sizes"
...
hg19)
    ...
    bwaIndex=/gpfs/data/isg/annotation/bwaIndex/hg19all/hg19all
...
mm10)
    ...
    referencefasta=/gpfs/data/isg/annotation/fasta/mm10_no_alt_analysis_set/mm10_no_alt_analysis_set.fa.gz
...
mm10all_CASTEiJ_female)
    bwaIndex=/gpfs/data/isg/annotation/bwaIndex/${mappedgenome}/${mappedgenome}
    ploidy="--ploidy-file /gpfs/data/isg/annotation/fasta/mm10_no_alt_analysis_set/mm10_no_alt_analysis_set.ploidy.txt"
    referencefasta=/gpfs/data/isg/annotation/bwaIndex/${mappedgenome}/${mappedgenome}.fa.gz
    dbsnpvcf=/dev/null
    #BUGBUG no dellyexclude for strain-specific references
    chromsizes="/gpfs/data/isg/annotation/bwaIndex/${mappedgenome}/${mappedgenome}.chrom.sizes"
...
rn6|rn6_sacCer3)
    bwaIndex=/gpfs/data/isg/annotation/bwaIndex/${mappedgenome}/${mappedgenome}
    ploidy="--ploidy-file /gpfs/data/isg/annotation/fasta/rn6/rn6.ploidy.txt"
    referencefasta=/gpfs/data/isg/annotation/fasta/${mappedgenome}/${mappedgenome}.fa.gz
    dbsnpvcf=/dev/null
    ;;
sacCer3)
...
talOcc4|talOcc4_sacCer3)
    bwaIndex=/gpfs/data/isg/annotation/bwaIndex/${mappedgenome}/${mappedgenome}
    ploidy="--ploidy-file /gpfs/data/isg/annotation/fasta/talOcc4/talOcc4.ploidy.txt"
    referencefasta=/gpfs/data/isg/annotation/fasta/${mappedgenome}/${mappedgenome}.fa.gz
    dbsnpvcf=/dev/null
    ;;
cegsvectors*)
    #If this genome is an symlink, then substitute it with its target for the purposes of looking up bwa index and fasta file
    bwagenome=`readlink -f /gpfs/data/cegs/sequences/${mappedgenome} | xargs basename`
    bwaIndex=/gpfs/data/cegs/sequences/${bwagenome}/${bwagenome}
    #In case this genome is an symlink,  use its target name to get the right chrom.sizes name
    chromsizes="/gpfs/data/cegs/sequences/${bwagenome}/${bwagenome}.chrom.sizes"
    ploidy="--ploidy 1"
    #Tried using the full cegsvectors.fa.gz but picard (esp CollectMultipleMetrics) has trouble with it)
    referencefasta=/gpfs/data/cegs/sequences/${bwagenome}/${bwagenome}.fa.gz
    dbsnpvcf=/dev/null
    ;;
t2t)
    bwaIndex=/gpfs/data/isg/annotation/bwaIndex/${mappedgenome}/${mappedgenome}
