...
for genome in "${genome_array[@]}"; do
    ...
    for curSection in Flowcells Aggregations Public_Data By_Locus; do
        if [ -f "${hub_target}/${genome}/trackDb.${curSection}.txt" ]; then
            ...
            awk -v max=160000 -v outbase="${hub_target}/${genome}/trackDb.${curSection}." 'BEGIN {wantToSplit=0; curChunk=1} {print > outbase "_" curChunk ".txt"} NR==max {wantToSplit=1} wantToSplit && $0~/^\s*$/ {curChunk+=1; wantToSplit=0}' ${hub_target}/${genome}/trackDb.${curSection}.txt
