...
if [[ "${analysisCommand}" == "callsnps" ]]; then
    ...
    if [[ "${analyzedReads}" == 0 ]]; then
        ...
        END {curOutputLine++; if(NR!=lastPrinted) {print lastChrom, firstStart, lastEnd, lastScore}}' > $TMPDIR/${name}.${mappedgenome}.coverage.bedGraph
    
        starch $TMPDIR/${name}.${mappedgenome}.coverage.bedGraph > ${sampleOutdir}/${name}.${mappedgenome}.coverage.starch
        ...
        samtools mpileup -u --min-BQ 20 --max-depth 10000 --redo-BAQ -f ${referencefasta} --BCF -t DP,AD ${sampleOutdir}/${name}.${mappedgenome}.bam |
        ...
        if [[ -f "${dbsnpvcf}" && "${dbsnpvcf}" != "/dev/null" ]]; then
            echo "Adding dbSNP IDs"
            date
            bcftools annotate --annotations ${dbsnpvcf} --columns ID --output-type v $TMPDIR/${name}.filtered.vcf.gz | 
            bgzip -c -@ $NSLOTS > ${sampleOutdir}/${name}.${mappedgenome}.filtered.vcf.gz
        else
            echo "No dbSNP IDs to add"
            date
            bcftools view --output-type v $TMPDIR/${name}.filtered.vcf.gz | 
            bgzip -c -@ $NSLOTS > ${sampleOutdir}/${name}.${mappedgenome}.filtered.vcf.gz
        fi
