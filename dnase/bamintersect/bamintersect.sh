...
## TMPDIR_bams is passed in via an sbatch export
########################################################
...
bam_intersect_data=$(tail -n+${SLURM_ARRAY_TASK_ID} "${TMPDIR_bams}/array_list" | head -n 1) || true

read -r bam1 bam2 outdir <<< ${bam_intersect_data}
echo ${bam1}
echo ${bam2}
echo ${outdir}      # "${TMPDIR_bamintersectPyOut}/${BASE1}___${BASE2}"
...
TMPDIR_bamintersect=`mktemp -d`    # A working directory for bamintersect.py, which is part of an array job.
"${src}/bamintersect.py" --bam1 ${bam1} --bam2 ${bam2} --outdir ${TMPDIR_bamintersect} ${same} ${make_csv_flag}

## See if there is anything in dsgrep_out.csv, which is produced by bamintersect.py
num_chars=$(wc -c < "${TMPDIR_bamintersect}/dsgrep_out.csv")
echo "num_chars -c is:  ${num_chars}"

## Let TMPDIR_bamintersect die if dsgrep_out.csv contains no output.
if [ ${num_chars} -ge 1 ]; then
    mv "${TMPDIR_bamintersect}" "${outdir}"
