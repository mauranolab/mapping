...
## OUTDIR is passed in via an sbatch export ($OUTDIR/OUTDIR_bams contains output of sort_bamintersect.sh)
########################################################
...
bam_intersect_data=$(tail -n+${SLURM_ARRAY_TASK_ID} "${OUTDIR}/OUTDIR_bams/inputs.array_list.txt" | head -n 1) || true

read -r bam1 bam2 outdir <<< ${bam_intersect_data}
echo ${bam1}
echo ${bam2}
echo ${outdir}      # "${OUTDIR}/bamintersectPyOut/${BASE1}___${BASE2}"
