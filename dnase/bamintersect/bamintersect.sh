...
## OUTDIR_bams is passed in via an sbatch export (contains output of sort_bamintersect.sh)
########################################################
...
bam_intersect_data=$(tail -n+${SLURM_ARRAY_TASK_ID} "${OUTDIR_bams}/inputs.array_list.txt" | head -n 1) || true

read -r bam1 bam2 outdir <<< ${bam_intersect_data}
echo ${bam1}
echo ${bam2}
echo ${outdir}      # "${OUTDIR_bamintersectPyOut}/${BASE1}___${BASE2}"
...
TMPDIR=`mktemp -d`    # A working directory for bamintersect.py, which is part of an array job.
