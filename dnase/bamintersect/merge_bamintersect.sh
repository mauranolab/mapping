...
dir_names=($(ls -d ${TMPDIR_bamintersectPyOut}/*/))      ## These have a trailing /

first="True"
for i in ${dir_names[@]}; do
    if [ ${first} = "True" ]; then
        cp "${i}dsgrep_out.csv" "${TMPDIR}/f1.bed"
        first="False"
    else
        cat "${i}dsgrep_out.csv" >> "${TMPDIR}/f1.bed"
    fi
done

date
echo Sorting...
sort-bed "${TMPDIR}/f1.bed" > "${sampleOutdir}/${sample_name}.bed"
date
...
while read main_chrom ; do
    ...
    mv "${sampleOutdir}/${sample_name}.${main_chrom}.informative.bed" ${TMPDIR}
    mv "${sampleOutdir}/${sample_name}.${main_chrom}.bed" ${TMPDIR}
