...
# Make the toplevel temp directory. TMPDIR has no trailing slash.
# This gets removed at the end of the merge_bamintersect.sh script.
TMPDIR=`mktemp -d --tmpdir=${sampleOutdir}`

# Subdirectories of this will hold output from bamintersect.py
TMPDIR_bamintersectPyOut="${TMPDIR}/bamintersectPyOut"
mkdir ${TMPDIR_bamintersectPyOut}

# Make a directory for all the small chromosome bam files.
TMPDIR_bams="${TMPDIR}/TMPDIR_bams"
mkdir ${TMPDIR_bams}
...
for BAM_N in $(seq 1 2); do
    ...
    while read chrom;  do
        ...
        BAM_OUT="${TMPDIR_bams}/${short_name}.${chrom}.${BAM_N}.bam"
        echo ${BAM_OUT} >> "${TMPDIR_bams}/chr_list_${BAM_N}"
...
while read -r bam1s; do
    while read -r bam2s; do
        ...
        # stdbuf --output=0 echo ${bam1s} ${bam2s} "${TMPDIR_bamintersectPyOut}/${BASE1}___${BASE2}"  >> "${TMPDIR}/TMPDIR_bams/array_list"
        echo ${bam1s} ${bam2s} "${TMPDIR_bamintersectPyOut}/${BASE1}___${BASE2}"  >> "${TMPDIR_bams}/array_list"

    done < "${TMPDIR_bams}/chr_list_2"
done < "${TMPDIR_bams}/chr_list_1"
...
export_vars="${export_vars},TMPDIR_bams=${TMPDIR_bams}"
...
echo "" >> "${sampleOutdir}/${sample_name}.counts.anc_info.txt"
echo "" >> "${sampleOutdir}/${sample_name}.counts.anc_info.txt"
echo "working file" > "${TMPDIR}/${sample_name}.counts.anc_info.txt"
...
export_vars="${export_vars},TMPDIR_bamintersectPyOut=${TMPDIR_bamintersectPyOut}"
export_vars="${export_vars},TMPDIR=${TMPDIR}"
