...
# The sample output directories will reside in the initial working directory.
outdirs=$(pwd)
...
# Derive some other required values from what we have.
    ...
    # outdirs was set to the initial working directory above.
...
OUTDIR=`mktemp -d --tmpdir=${sampleOutdir}`   # OUTDIR contains no trailing slash.

# Subdirectories of OUTDIR_bamintersectPyOut will hold output from bamintersect.py
OUTDIR_bamintersectPyOut="${OUTDIR}/bamintersectPyOut"
mkdir ${OUTDIR_bamintersectPyOut}

# Make a directory for all the small sorted, chromosome bam files generated by sort_bamintersect.sh
OUTDIR_bams="${OUTDIR}/OUTDIR_bams"
mkdir ${OUTDIR_bams}
...
for BAM_N in $(seq 1 2); do
    ...
    while read chrom;  do
        ...
        BAM_OUT="${OUTDIR_bams}/${short_name}.${chrom}.${BAM_N}.bam"
        echo ${BAM_OUT} >> "${OUTDIR_bams}/chr_list_${BAM_N}"
...
while read -r bam1s; do
    while read -r bam2s; do
        ...
        # stdbuf --output=0 echo ${bam1s} ${bam2s} "${OUTDIR_bamintersectPyOut}/${BASE1}___${BASE2}"  >> "${TMPDIR}/TMPDIR_bams/inputs.array_list.txt"
        echo ${bam1s} ${bam2s} "${OUTDIR_bamintersectPyOut}/${BASE1}___${BASE2}"  >> "${OUTDIR_bams}/inputs.array_list.txt"

    done < "${OUTDIR_bams}/chr_list_2"
done < "${OUTDIR_bams}/chr_list_1"
...
export_vars="${export_vars},OUTDIR_bams=${OUTDIR_bams}"
...
JOB_ID0=$(sbatch --qos=low --parsable --export=ALL,${export_vars} --array="1-${num_lines}" --job-name=sort_bamintersect \
...
JOB_ID1=$(sbatch --qos=low --parsable --export=ALL,${export_vars} --array="1-${num_lines}" --job-name=sort_bamintersect \
...
samtools idxstats ${bamname1} > "${OUTDIR}/${sample_name}.counts.anc_info.txt"
num_lines=$(wc -l < "${OUTDIR}/${sample_name}.counts.anc_info.txt")
...
head "-${num_lines}" "${OUTDIR}/${sample_name}.counts.anc_info.txt" >> "${sampleOutdir}/${sample_name}.counts.anc_info.txt"
echo "" >> "${sampleOutdir}/${sample_name}.counts.anc_info.txt"
echo "" >> "${sampleOutdir}/${sample_name}.counts.anc_info.txt"
echo "working file" > "${OUTDIR}/${sample_name}.counts.anc_info.txt"
...
export_vars="${export_vars},OUTDIR_bamintersectPyOut=${OUTDIR_bamintersectPyOut}"
export_vars="${export_vars},OUTDIR=${OUTDIR}"

JOB_ID2=$(sbatch --parsable --dependency=afterok:${JOB_ID1} --export=ALL,${export_vars} --job-name=merge_bamintersect \
