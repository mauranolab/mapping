...
for BAM_N in $(seq 1 2); do
    ...
    while read chrom;  do
        ...
        BAM_OUT="${OUTDIR}/OUTDIR_bams/${short_name}.${chrom}.${BAM_N}.bam"
        echo ${BAM_OUT} >> "${OUTDIR}/OUTDIR_bams/chr_list_${BAM_N}"
...
while read -r bam1s; do
    while read -r bam2s; do
        ...
        # stdbuf --output=0 echo ${bam1s} ${bam2s} "${OUTDIR}/bamintersectPyOut/${BASE1}___${BASE2}"  >> "${TMPDIR}/TMPDIR_bams/inputs.array_list.txt"
        echo ${bam1s} ${bam2s} "${OUTDIR}/bamintersectPyOut/${BASE1}___${BASE2}"  >> "${OUTDIR}/OUTDIR_bams/inputs.array_list.txt"

    done < "${OUTDIR}/OUTDIR_bams/chr_list_2"
done < "${OUTDIR}/OUTDIR_bams/chr_list_1"
