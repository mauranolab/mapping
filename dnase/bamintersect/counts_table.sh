...
# However, keep nearby bam2 reads segregated by bam1 chromosome name, by prepending the bam1 chromosome name to the bam2 chromosome name.
awk -F "\t" 'BEGIN {OFS="\t"} {print $1"|"$7,$8,$9}' ${bamintersectBED12} | sort-bed - > ${TMPDIR}/sorted_speciesReadCoords.bed3

# The below makes +/- 500 bps regions around each segregated hg38/mm10 read, and merges them when possible. It will return bed3 output.
bedops --range 500 -m ${TMPDIR}/sorted_speciesReadCoords.bed3 |
# Undo the prepend operation, since closest-features needs the real bam2 name. Preserve the bam1 name at the end of the string.
awk -F "|" 'BEGIN {OFS="\t"} {print $2,$1}' | sort-bed - > ${TMPDIR}/all_regions.bed
...
# Prepend the bam1 name again, since that is how it is in sorted_speciesReadCoords.
awk -F "\t" 'BEGIN {OFS="\t"} {print $4"|"$0}' | sort-bed - |
# Count the number of reads in each segregated region.
...
$6>=minReadsCutoff { \
   ...
   width = $3 - $2 - 1000; \
   print $1, chromStart, chromEnd, width, $5, $6, $4; \
}' |
#Sort by Post-filter_Reads, then chromosome-bam2, then Start_Pos
sort -V -t $'\t' -k 6,6rn -k 1,1 -k 2,2n - |
# Append the short sample name onto the last column.
awk -v short_sample_name=`echo "${sample_name}" | cut -d "." -f1` -F "\t" 'BEGIN {OFS="\t"} {print $0, short_sample_name}' |
# Undo the prepend operation.
awk -F "|" 'BEGIN {OFS="\t"} {print $2}' > ${TMPDIR}/short_sorted_table.bed
