...
annotateNearestGeneName() {
    ...
    *)
    ...
    if [[ "${geneAnnotationFile}" != "CEGS" ]]; then
        ...
        cat /gpfs/data/cegs/sequences/cegsvectors/cegsvectors/cegsvectors.bed6 | perl -pe 's/ /_/g;' > $TMPDIR/geneAnnotationFile.${genome}.bed
    fi
...
for strand_bam1 in "+" "-"; do
    for strand_bam2 in "+" "-"; do
        ...
        awk -v outdir=$TMPDIR/bam2readsByBam1region_${strand_bam1}_${strand_bam2} -F "|" '
        BEGIN {OFS="\t"} {split($1, bam1region, "\t");
        for(i=2; i<=NF; i++) {
            split($i, bam2read, "\t"); print bam2read[7], bam2read[8], bam2read[9], bam1region[4] > outdir "/" NR ".unsorted.bam2.bed"
        }}'
