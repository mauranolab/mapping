...
find . -regextype posix-awk -maxdepth 2 -regex ".+\/[^\/]+(BS|SRR)[^\/]+\/.+" -and -name analysis.*.o* | xargs grep -h -A 11 "Num_sequenced_reads" | awk 'BEGIN {skip=0} $0=="--" {skip=0;next} $0=="" {skip=1;next} skip==0' | grep -v -e "syntax[ _]error" | grep -v "Num_SE_" | grep -v "Prop_SE_" | 
#awk '$1!="$1!="Num_hotspots" && SPOT"' |
perl -p -e's/[ \t]\(?([\d\.]+)\%\)?/\t$1/g;' -e's/ /_/g;' -e's/\-(BS|SRR)/\t\1/g;' | 
