...
awk -F "\t" 'BEGIN {OFS="\t"; print "Label", "Num", "Pct", "Cell_type", "BS", "Genome"} {if(NF==4) {$5="NA"; $6="NA"} else if (NF==5) {$6="NA"} print}' | cut -f1-6 > /tmp/t.txt
...
colnames(data) <- gsub("^Num\\.Num_", "Num_", colnames(data))
colnames(data) <- gsub("^Pct\\.Num_", "Prop_", colnames(data))
...
colnames(data) <- gsub("^Num\\.SPOT(2?)", "SPOT\\1", colnames(data))
data <- data[order(data[,"BS"]),]
