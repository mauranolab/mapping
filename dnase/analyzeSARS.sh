...
for bamfile in `find ${dir} -name "*.bam" -not -name "*R1R2*"`; do
    ...
    base=`basename ${bamfile} .hg38_full_wuhCor1.bam`
    ...
    covfile=`echo ${bamfile} | perl -pe 's/\.bam$/.coverage.starch/g;'`
    unstarch NC_045512v2 ${covfile} | awk '$5>=20' | awk -F "\t" 'BEGIN {sum=0} {sum+=$3-$2} END {print sum}'
