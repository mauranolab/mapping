...
def dedup_dir_adj(Counter):
		def get_adj_list_directional_adjacency(umis, counts):
			return {umi: [umi2 for umi2 in umis if levenshtein(umi, umi2) == 1 and
									counts[umi] >= (counts[umi2]*2)-1] for umi in umis} 

##Switch order of loop
#def dedup_dir_adj(Counter):
#		def get_adj_list_directional_adjacency(umis, counts):
#						 counts[umi] >= (counts[umi2]*2)-1] and
#						return {umi: [umi2 for umi2 in umis if edit_distance(umi, umi) <= 1] 
#						for umi in umis}
#		
