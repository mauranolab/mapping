...
#             UmiEncoded = [umi.encode() for umi in umis] #Encode all umis form start in list
...
#             	umi = umisKeys[i]#get keys from dict_keys
#             	umiCounts = counts[umi]
#             	umiCode = UmiEncoded[i]
#             	nearbyUmis = [] #add Umi as key in list
#             	for j in range(1, len(umis)): #List comprehension here #TODO focus here (Filter) #List comprehension
#             		umi2 = umisKeys[j]
#             		umi2Code = UmiEncoded[j]
#             		if umi != umi2 and umiCounts >= (counts[umi2]*2)-1 and edit_distance(umiCode, umi2Code) == 1:
#             			nearbyUmis.append(umi2) #If this take time, preallocate #TODO fix append https://wiki.python.org/moin/PythonSpeed/PerformanceTips#Loops
