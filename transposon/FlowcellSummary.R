...
outputCols <- c("Flowcell", "BS", "Name", "Type", "Total read pairs", "Total barcodes", "BC+UMI", "UMI length", "Unique BC", "BC length", "Complexity", "Mapped reads", "Mapped+BC reads", "Unique sites", "Unique sites +-5bp", "#BC 1 site", "#BC 2+ sites", "Prop 2+")
...
#		}
		...
		if(any(grepl('Total PF tags', Sample))) {
			data[i, "Mapped reads"] <- as.numeric(splitLines('Total PF tags', '\t')$X3)
			data[i, "Mapped+BC reads"] <- as.numeric(splitLines('Number of tags passing all filters and having barcodes assigned', '\t')$X2)
			data[i, "Unique sites"] <- as.numeric(splitLines('Total uniq sites', '\t')$X3[1])
			data[i, "Unique sites +-5bp"] <- as.numeric(splitLines('Total uniq sites (within 5 bp, ignoring strand)', '\t')$X3[1])
...
SumHTMLtable <- tableHTML(data, rownames=F, footer='N.B.: Unique BC requires 10+ reads, integration sites require 2+ reads') %>% add_css_row(css = list('background-color', 'lightblue'), rows = odd(1:nrow(data)))
